...
The `version` property is optional. It's informational and only potentially used for flagging if a checked out version
is not
the specified version, so it can be a way to be alerted that something changed.
