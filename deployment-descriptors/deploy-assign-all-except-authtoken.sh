...
curl -w '\n' -D - -s -X POST -H "Content-type: application/json" -d @$workdir/deployment-descriptors/DeploymentDescriptor-mod-users-14.2.2-SNAPSHOT.json http://localhost:9130/_/discovery/modules
...
curl -w '\n' -D - -s -X POST -H "Content-type: application/json" -d @$workdir/deployment-descriptors/DeploymentDescriptor-mod-users-bl-2.1.1-SNAPSHOT.json http://localhost:9130/_/discovery/modules
